...
namespace FloydPink.Flickr.Downloadr.UI.Behaviors {
    ...
    public class TwoListSynchronizer : IWeakEventListener {
        ...
        public void StartSynchronizing() {
            ListenForChangeEvents(this._masterList);
            ListenForChangeEvents(this._targetList);
            ...
            if (!TargetAndMasterCollectionsAreEqual()) {
                SetListValuesFromSource(this._targetList, this._masterList, ConvertFromTargetToMaster);
            }
                // Update the Target list from the Master list
            else {
                SetListValuesFromSource(this._targetList, this._masterList, ConvertFromMasterToTarget);
            }
        }
        ...
        public void StopSynchronizing() {
            StopListeningForChangeEvents(this._masterList);
            StopListeningForChangeEvents(this._targetList);
        }
        ...
        private void PerformActionOnAllLists(ChangeListAction action, IList sourceList,
            ...
            if (sourceList == this._masterList) {
                PerformActionOnList(this._targetList, action, collectionChangedArgs, ConvertFromMasterToTarget);
            } else {
                PerformActionOnList(this._masterList, action, collectionChangedArgs, ConvertFromTargetToMaster);
            }
        }
        ...
        private bool TargetAndMasterCollectionsAreEqual() {
            return
                this._masterList.Cast<object>()
                    .SequenceEqual(this._targetList.Cast<object>().Select(item => ConvertFromTargetToMaster(item)));
        }
        ...
        private void UpdateListsFromSource(IList sourceList) {
            if (sourceList == this._masterList) {
                SetListValuesFromSource(this._masterList, this._targetList, ConvertFromMasterToTarget);
            } else {
                SetListValuesFromSource(this._targetList, this._masterList, ConvertFromTargetToMaster);
            }
        }
